...
func SetKeys(
	...
	connRess := make(map[object.ID][]*model.ResourceComponent)
	{
		...
		for _, c := range candidates {
			...
			if c.Shape == types.ResourceComponentShapeClass {
		...
		for id := range caps {
			connRess[id] = make([]*model.ResourceComponent, 0, caps[id])
		}
	...
	opRess := make(map[optypes.Operator][]*model.ResourceComponent)
	{
		...
		for i := 0; i < bks; i++ {
			...
			wg.Go(func(ctx context.Context) error {
				...
				for j := s; j < len(ress); j += bks {
					if ress[j].Shape != types.ResourceComponentShapeInstance ||
						ress[j].Mode == types.ResourceComponentModeData {
						continue
					}
