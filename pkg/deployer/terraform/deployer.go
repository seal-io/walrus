...
func (d Deployer) Apply(ctx context.Context, service *model.Service, opts deptypes.ApplyOptions) (err error) {
	return d.createJob(ctx, service, JobTypeApply, opts.SkipTLSVerify)
}

// Destroy will destroy the resource of the service.
// 1. Get the latest revision, and check it if is running.
...
func (d Deployer) Refresh(
	...
	return d.createJob(ctx, service, JobTypeRefresh, opts.SkipTLSVerify)
}
...
func (d Deployer) Detect(
	...
	return d.createJob(ctx, service, JobTypeDetectDrift, opts.SkipTLSVerify)
}

func (d Deployer) createJob(ctx context.Context, svc *model.Service, jobType string, skipTLSVerify bool) error {
	sr, err := d.CreateServiceRevision(ctx, CreateRevisionOptions{
		JobType: jobType,
		Service: svc,
	})
