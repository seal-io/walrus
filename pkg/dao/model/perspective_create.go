...
func (pc *PerspectiveCreate) check() error {
	...
	if _, ok := pc.mutation.CreateTime(); !ok {
		return &ValidationError{Name: "create_time", err: errors.New(`model: missing required field "Perspective.create_time"`)}
	}
	if _, ok := pc.mutation.UpdateTime(); !ok {
		return &ValidationError{Name: "update_time", err: errors.New(`model: missing required field "Perspective.update_time"`)}
	}
	if _, ok := pc.mutation.StartTime(); !ok {
		return &ValidationError{Name: "start_time", err: errors.New(`model: missing required field "Perspective.start_time"`)}
	}
	...
	if _, ok := pc.mutation.EndTime(); !ok {
		return &ValidationError{Name: "end_time", err: errors.New(`model: missing required field "Perspective.end_time"`)}
	}
...
//	   db.SetX(obj.X)
//	}
func (pc *PerspectiveCreate) Set(obj *Perspective) *PerspectiveCreate {
	// Required.
	pc.SetName(obj.Name)
...
func (pc *PerspectiveCreate) getClientSet() (mc ClientSet) {
...
// SaveE calls the given function after created the Perspective entity,
...
func (pc *PerspectiveCreate) SaveE(ctx context.Context, cbs ...func(ctx context.Context, mc ClientSet, created *Perspective) error) (*Perspective, error) {
	...
	if x := pc.object; x != nil {
		...
		if _, set := pc.mutation.Field(perspective.FieldDescription); set {
			obj.Description = x.Description
		}
		if _, set := pc.mutation.Field(perspective.FieldStartTime); set {
			obj.StartTime = x.StartTime
		}
		if _, set := pc.mutation.Field(perspective.FieldEndTime); set {
...
// Set leverages the PerspectiveCreate Set method,
...
//	   db.SetX(obj.X)
//	}
func (pcb *PerspectiveCreateBulk) Set(objs ...*Perspective) *PerspectiveCreateBulk {
	if len(objs) != 0 {
		client := NewPerspectiveClient(pcb.config)

		pcb.builders = make([]*PerspectiveCreate, len(objs))
		for i := range objs {
...
// getClientSet returns the ClientSet for the given builder.
func (pcb *PerspectiveCreateBulk) getClientSet() (mc ClientSet) {
...
// SaveE calls the given function after created the Perspective entities,
// which is always good for cascading create operations.
func (pcb *PerspectiveCreateBulk) SaveE(ctx context.Context, cbs ...func(ctx context.Context, mc ClientSet, created *Perspective) error) ([]*Perspective, error) {
	...
	if x := pcb.objects; x != nil {
		for i := range x {
			if _, set := pcb.builders[i].mutation.Field(perspective.FieldName); set {
				objs[i].Name = x[i].Name
			}
			if _, set := pcb.builders[i].mutation.Field(perspective.FieldDescription); set {
				objs[i].Description = x[i].Description
			}
			if _, set := pcb.builders[i].mutation.Field(perspective.FieldStartTime); set {
				objs[i].StartTime = x[i].StartTime
			}
			if _, set := pcb.builders[i].mutation.Field(perspective.FieldEndTime); set {
...
func (u *PerspectiveUpsertOne) ExecE(ctx context.Context, cbs ...func(ctx context.Context, mc ClientSet, updated *Perspective) error) error {
	if len(u.create.conflict) == 0 {
		return errors.New("model: missing options for PerspectiveUpsertOne.OnConflict")
	}
...
func (u *PerspectiveUpsertBulk) ExecE(ctx context.Context, cbs ...func(ctx context.Context, mc ClientSet, updated *Perspective) error) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("model: OnConflict was set for builder %d. Set it on the PerspectiveUpsertBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("model: missing options for PerspectiveUpsertBulk.OnConflict")
	}
	u.create.fromUpsert = true
	return u.create.ExecE(ctx, cbs...)
}
