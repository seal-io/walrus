...
func (src *ServiceRelationshipCreate) Save(ctx context.Context) (*ServiceRelationship, error) {
	...
	return withHooks[*ServiceRelationship, ServiceRelationshipMutation](ctx, src.sqlSave, src.mutation, src.hooks)
}
...
func (src *ServiceRelationshipCreate) check() error {
	if _, ok := src.mutation.CreateTime(); !ok {
		return &ValidationError{Name: "createTime", err: errors.New(`model: missing required field "ServiceRelationship.createTime"`)}
	}
