...
func getPostgreSQLFlexibleServer(ctx context.Context, resourceType, name string) (*status.Status, error) {
	...
	client, err := armpostgresqlflexibleservers.NewServersClient(cred.SubscriptionID, sc, nil)
	if err != nil {
		return nil, err
	}
	...
	return postgreSQLFlexibleServerStatusConverter.Convert(string(*resp.Properties.State), ""), nil
}
