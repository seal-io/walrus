...
func getMySQLFlexibleServer(ctx context.Context, resourceType, name string) (*status.Status, error) {
	...
	client, err := armmysqlflexibleservers.NewServersClient(cred.SubscriptionID, sc, nil)
	if err != nil {
		return nil, err
	}
	...
	return mySQLFlexibleServerStatusConverter.Convert(string(*resp.Properties.State), ""), nil
}
