...
func getRedisCache(ctx context.Context, resourceType, name string) (*status.Status, error) {
	...
	client, err := armredis.NewClient(cred.SubscriptionID, sc, nil)
	if err != nil {
		return nil, err
	}
	...
	return redisCacheStatusConverter.Convert(string(*resp.Properties.ProvisioningState), ""), nil
}
