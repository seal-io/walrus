...
func NewMemoryWithConfig(ctx context.Context, cfg MemoryConfig) (Cache, error) {
	...
	//
	// example bigcache config:
	// bigcache.Config{
	//		LifeWindow:         15 * time.Minute,
	//		CleanWindow:        3 * time.Minute,
	//		Shards:             64,
	//		MaxEntriesInWindow: 64 * 5 * 60,  // works with MaxEntrySize to determinate the cache initialization.
	//		MaxEntrySize:       512,
	//		HardMaxCacheSize:   64,
	//		StatsEnabled:       false,
	//		Verbose:            false,
	...
	//
	var capacity = cfg.BucketCapacity * cfg.Buckets
	var logger = log.WithName("cache").WithName("memory")
	var underlayCfg = bigcache.Config{
		Shards:             cfg.Buckets,
		LifeWindow:         cfg.EntryMaxAge,
		CleanWindow:        0,
		MaxEntriesInWindow: cfg.Buckets,
...
func MustNewMemoryWithConfig(ctx context.Context, cfg MemoryConfig) Cache {
	var n, err = NewMemoryWithConfig(ctx, cfg)
	if err != nil {
		panic(fmt.Errorf("error creating in-memory cache: %w", err))
	}
	return n
