...
type Encryptor interface {
	...
	Encrypt(plaintext, additional []byte) (ciphertext []byte, err error)

	// Decrypt obtains the plaintext bytes with the given bytes,
	// additional data can be nil.
	Decrypt(ciphertext, additional []byte) (plaintext []byte, err error)
}
...
func (nullEncryptor) Encrypt(p, _ []byte) ([]byte, error) {
	if p == nil {
		return p, nil
	}
...
func (nullEncryptor) Decrypt(p, _ []byte) ([]byte, error) {
	if p == nil {
		return p, nil
	}
