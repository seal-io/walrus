...
# following variables:
#
#           PROTOC_VERSION  -  The protoc version, default is v3.20.1.
#       PROTOC_GEN_VERSION  -  The protoc-gen-go version, default is v1.28.
#  PROTOC_GEN_GRPC_VERSION  -  The protoc-gen-go-grpc version, default is v1.28.
#
# Refs:
# - https://grpc.io/docs/protoc-installation/
# - https://grpc.io/docs/languages/go/quickstart/

protoc_version=${PROTOC_VERSION:-"v3.20.1"}
protoc_gen_version=${PROTOC_GEN_VERSION:-"v1.28"}
protoc_gen_grpc_version=${PROTOC_GEN_GRPC_VERSION:-"v1.2"}
...
function seal::protoc::protoc_gen::install() {
  local bin="${ROOT_DIR}/.sbin/protoc/bin"
  mkdir -p "${bin}"
  GOBIN="${bin}" go install "google.golang.org/protobuf/cmd/protoc-gen-go@${protoc_gen_version}"
}

function seal::protoc::protoc_gen::validate() {
  PATH="${PATH}:${ROOT_DIR}/.sbin/protoc/bin"
  if [[ -f "${ROOT_DIR}/.sbin/protoc/bin/protoc-gen-go" ]]; then
    return 0
  fi

  seal::log::info "installing protoc-gen ${protoc_gen_version}"
  if seal::protoc::protoc_gen::install; then
    seal::log::info "protoc-gen installed"
    return 0
  fi
  seal::log::error "no protoc-gen available"
  return 1
}

function seal::protoc::protoc_gen_grpc::install() {
  local bin="${ROOT_DIR}/.sbin/protoc/bin"
  mkdir -p "${bin}"
  GOBIN="${bin}" go install "google.golang.org/grpc/cmd/protoc-gen-go-grpc@${protoc_gen_grpc_version}"
}

function seal::protoc::protoc_gen_grpc::validate() {
  PATH="${PATH}:${ROOT_DIR}/.sbin/protoc/bin"
  if [[ -f "${ROOT_DIR}/.sbin/protoc/bin/protoc-gen-go-grpc" ]]; then
    return 0
  fi

  seal::log::info "installing protoc-gen-grpc ${protoc_gen_grpc_version}"
  if seal::protoc::protoc_gen_grpc::install; then
    seal::log::info "protoc-gen-grpc installed"
    return 0
  fi
  seal::log::error "no protoc-gen-grpc available"
  return 1
}
...
function seal::protoc::generate() {
  ...
  for generated_file in "${generated_files[@]}"; do
    if [[ -f "${generated_file}" ]]; then
      ...
      cat "${ROOT_DIR}/hack/boilerplate/protoc.txt" "${generated_file}" >"${tmpfile}" &&
        mv "${tmpfile}" "${generated_file}"
